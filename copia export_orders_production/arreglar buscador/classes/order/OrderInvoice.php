...
class OrderInvoiceCore extends ObjectModel
{
	 ...
	 *@return string
	 */
	public function getMessagesByOrderIdPublic($id_order,$idprivate)
	{
		$messages='';
		$result = Db::getInstance()->executeS('
		SELECT `message`
		FROM `ps_customer_message` cm
		LEFT JOIN `ps_customer_thread` ct ON ct.`id_customer_thread` = cm.`id_customer_thread`
		WHERE `id_order` = '.$id_order.' AND `private`='.$idprivate.' 
		');
		foreach ($result as $row) {
	 ...
	 *@return string
	 */
	public function getMessagesByOrderIdPri($id_order,$idprivate)
	{
		$messages='';
		$result = Db::getInstance()->executeS('
		SELECT `message`
	 ...
	 *@return string
	 */
	public function getNoteCustomer($id_customer)
	{
		return Db::getInstance()->getValue('
		SELECT `note`
		FROM `ps_customer` 
