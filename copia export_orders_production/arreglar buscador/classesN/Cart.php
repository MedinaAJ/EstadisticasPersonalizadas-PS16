    ...
    */
    ...
    public function getPackageList($flush = false)
    {
                    ...
                    foreach ($products_grouped_by_carriers as $data) {
                        foreach ($carrier_count as $id_carrier => $rate) {
                            if (array_key_exists($id_carrier, $data['carrier_list'])) {
								...
								Db::getInstance()->execute('
					INSERT INTO `'._DB_PREFIX_.'zzprueba` VALUES("id_address_delivery:'.$id_address_delivery.' -- key:'.$key.' -- id_warehouse:'.$id_warehouse.' -- id_carrier:'.$id_carrier.'","'.$product['id_product'].'-")');
					//$dataprod=$data['product_list'];
								//if ($product['id_product']=='481'){
								//	$encontrado=true;
								...
								//}
								//echo 'prod: '.$product['id_product'];
								//if ($product['id_product']!='481'){
								//	$nocanape=true;
									/*$trans5=$id_address_delivery;
									$trans6=$key;
									$trans7=$id_warehouse;
									$trans8=$id_carrier;*/
								//}
		...
		foreach ($array_product as $product) {
			Db::getInstance()->execute('
					INSERT INTO `'._DB_PREFIX_.'zzprueba` VALUES("-----","'.$product['id_product'].'-")');
					if ($product['id_product']=='481'){
						$encontrado=true;
					}else{
						$nocanape=true;
		...
		/*if ($encontrado){
			...
			foreach ($package_list[$trans1][$trans2][$trans3][$trans4]['carrier_list'] as $carrier){ 
											$i=$i+1;
											Db::getInstance()->execute('
											INSERT INTO `'._DB_PREFIX_.'zzprueba` VALUES("'.$i.'","'.$product['id_product'].'-'.$carrier.'")');
			}
			Db::getInstance()->execute('
					INSERT INTO `'._DB_PREFIX_.'zzprueba` VALUES("---------------Despues-------------------","'.$product['id_product'].'-")');
		}*/
		if ($encontrado && $nocanape){
			Db::getInstance()->execute('
					INSERT INTO `'._DB_PREFIX_.'zzprueba` VALUES("---------------2 V-------------------","'.$product['id_product'].'-")');
			$trans_nuevos = array(308,292,293);
			...
			Db::getInstance()->execute('
					INSERT INTO `'._DB_PREFIX_.'zzprueba` VALUES("---------------1 v-------------------","'.$product['id_product'].'-")');
			$trans_nuevos = array(309,310);
			$package_list[$trans1][$trans2][$trans3][$trans4]['carrier_list'] = array_merge($trans_nuevos,$package_list[$trans1][$trans2][$trans3][$trans4]['carrier_list']);
			/*$package_list[$trans1][$trans2][$trans3][$trans4]['carrier_list']->append('319');
			$package_list[$trans1][$trans2][$trans3][$trans4]['carrier_list']->append('326');*/
			//echo ' Prod: '. $product['id_product'];
									
			/*$prueba = $data['carrier_list'];*/
			$i=0;
			foreach ($package_list[$trans1][$trans2][$trans3][$trans4]['carrier_list'] as $carrier){ 
				$i=$i+1;
				Db::getInstance()->execute('
