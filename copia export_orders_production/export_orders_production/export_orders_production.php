...
//************************************************************************************************************************
	{   global $cookie;
        ...
        if(Tools::isSubmit('submitSetting')){
            ...
            $xr = rtrim($xr,'↔');
            $xr .= '‡';
			//echo '<br/><br/>CSV: '. Tools::getValue('icsv');
...
$filterNoInIds = '';
if ($df['ESPUMA']){
	$sql='SELECT DISTINCT id_order FROM '._DB_PREFIX_.'order_history WHERE id_order_state=207'. $filterInIds.';';
	if($resNoId = Db::getInstance()->ExecuteS($sq)){
		$filterNoInIds = ' AND id_order NOT IN(';	
...
$sq = 'SELECT id_order FROM '._DB_PREFIX_.'orders WHERE id_order  > '.$df['LASTIDORDER'].$filterFr.$filterTo.$filterMshop.$filterInv.$filterInIds.$filterNoInIds.' ORDER BY id_order ASC'.$oneorder1file;
...
return $out; } ;
				...
				if (count($resmp)>0){
					...
					foreach($resmp as $imp){
						...
						if (count($rescust)>0){
							...
							//print('<br/>sql con resultados');
							if ($imp['width']=='80' or $imp['width']=='90' or $imp['width']=='105' or $imp['width']=='120' or $imp['width']=='135' or $imp['width']=='140' or $imp['width']=='150' or $imp['width']=='160' or $imp['width']=='180' or $imp['width']=='200'){
								if ($imp['height']=='180' or $imp['height']=='190' or $imp['height']=='200'){
									if($imp['length']=='0'){
										$imp['attributes'] =$imp['attributes'] . '-CUIDADO MEDIDA ESTÁNDAR';
									}
								}
							}
							$dafi=$this->toDafiRow($dafi,$imp);
					...
					//print('<br/>sql sin resultados 1');
					$sq = "SELECT ".$this->myFields($tabf,'cust')."a.id_product,a.id_customization,(@rownum:=@rownum+1) AS posicion FROM (SELECT @rownum:=0) R, "._DB_PREFIX_."customization a, "._DB_PREFIX_."customized_data cust, "._DB_PREFIX_."cart_product c WHERE a.id_customization=cust.id_customization AND a.id_cart=c.id_cart AND a.id_product=c.id_product AND c.id_cart='" .$resnuevo['id_cart']."' and a.id_product=".$iod['product_id'];
					$rescust = DB::getInstance()->ExecuteS($sq);
					//print('<br/>'.$sq);
					if (count($rescust)>0){
						//print('<br/>sql con resultados 2');
						$attr = array('attributes'=>'','length'=>'','width'=>'','height'=>'','personalization'=>'','quantity'=>'');
						foreach($rescust as $cust){
							$attr['attributes'] =$attr['attributes'] . $cust['customized_data'].' ';
						}
						...
						$attr = array('attributes'=>'','length'=>'','width'=>'','height'=>'','personalization'=>'','quantity'=>'');
						$dafi=$this->toDafiRow($dafi,$attr);
						//print('<br/>sql sin resultados 2');
						//******************************************************************************
						//******************************************************************************
						//******************************************************************************
						//echo '<br/>'.$this->myFields($tabf,'idxr');
						$sq='Select '.$this->myFields($tabf,'idxr').'idxr.id_note,(@rownum:=@rownum+1) AS posicion from (SELECT @rownum:=0) R, '._DB_PREFIX_.'idxrcustomproduct_notes as idxr where idxr.id_order='.$iod['id_order'].' and idxr.id_cart_product='.$iod['product_id'];
						//echo '<br/>hola: ' . $sq . '<br/>';
						$residx = Db::getInstance()->ExecuteS($sq);
						if (count($residx)>0){
							foreach($residx as $imp){
								$dafi=$this->toDafiRow($dafi,$imp);
								$megapole[]=$dafi;
							}
						}else{
							$imp = array('public_note'=>'');
							$dafi=$this->toDafiRow($dafi,$imp);
							$megapole[]=$dafi;
						}
					}
		...
		if($df['SAVEAS'] == 'xlsx'){
			...
			if(!$fe){//neexistuje
				...
				$objxlsx->getActiveSheet()->getColumnDimensionByColumn(24)->setWidth(0);
		...
		foreach($megapole as $mk=>$mv){
			...
			if($df['SAVEAS'] == 'xlsx'){
				...
				foreach($ki as $kk=>$kv){
					...
					if ($kk=='attributes'){
						$val=explode('-',$kv);
						//print('<br/>total: '.count($val).' '. $val[0]);
						$kva='';
						...
						//echo '<br/>Articulo: '.strtolower($this->stripAccents($kv));
						if (strpos(strtolower($this->stripAccents($kv)),'oferton') || strpos(strtolower($this->stripAccents($kv)),'cod')){
							$advertencia='OJO! NO HACER,ARTÍCULO DE OUTLET. ';
						}else{
							if(strpos(strtolower($this->stripAccents($kv)),'exposicion')){
						...
						if (strpos(strtolower($this->stripAccents($kv)),'si (+30')){
						...
						$objxlsx->getActiveSheet()->setCellValueByColumnAndRow(6,$xr,$advertencia.$kv); 	
						;  	
						
					} 
					if($kk=='public_note'){
						$kvx = str_replace("<p>","",$kv);
						$kvx = str_replace("</p>","-",$kv);
						$xvx= explode("-",$kvx);
						$result="";
						foreach($xvx as $s){
						...
						$kvx=$result;
						$kv=$objxlsx->getActiveSheet()->getCellByColumnAndRow(6, $xr)->getFormattedValue(). ' ' .$kvx;
					...
					if ($kk=='order_status'){
						if ($kv=="Producción CS"){
							...
							$kv = 'OJO!! RECOGIDA EN FÁBRICA =>> ' . $objxlsx->getActiveSheet()->getCellByColumnAndRow(12, $xr)->getFormattedValue();
					...
					if ($kk=='carrier_name'){
						...
						if ($kv=='Subida y montaje de los artículos'){
							$kv="MOB"; 
						}
					...
					$advertencia = '';
						...
						if (($kk=='public_note')){
							...
							if($df['COSTURA']){
								...
								if (strpos($kvmdf, "cabecero")!==false or strpos($kvmdf, "canape")!==false or strpos($kvmdf, "baul")!==false or strpos($kvmdf, "monaco")!==false or strpos($kvmdf, "base")!==false or strpos($kvmdf, "colchon")!==false or strpos($kvmdf, "funda")!==false or strpos($kvmdf, "strecht")!==false or strpos($kvmdf, "topper")!==false or strpos($kvmdf, "powerloom")!==false or strpos($kvmdf, "polipiel")!==false or strpos($kvmdf, "cuna")!==false or strpos($kvmdf, "cambiadores")!==false or strpos($kvmdf, "harrier")!==false or strpos($kvmdf, "loneta")!==false or strpos($kvmdf, "tela")!==false or strpos($kvmdf, "premium")!==false){	
									$kv = str_replace("CR~LF","\n",$kv);
									if(is_numeric($ki[$kk]) AND $ktyp[$kk] == 3){
										$objxlsx->getActiveSheet()->setCellValueExplicitByColumnAndRow($i,$xr,$advertencia .$kv, PHPExcel_Cell_DataType::TYPE_STRING); 
									}else{
						...
						$kva= $advertencia .$kv;
						if(is_numeric($ki[$kk]) AND $ktyp[$kk] == 3){
							$objxlsx->getActiveSheet()->setCellValueExplicitByColumnAndRow($i,$xr,$kva, PHPExcel_Cell_DataType::TYPE_STRING); 
						}else{	
							$objxlsx->getActiveSheet()->setCellValueByColumnAndRow($i,$xr,$kva); 
						}
					}
		            $i++;
				}
		...
		if($df['SAVEAS'] == 'xlsx'){
			...
			$col = 1;
			$row = 1;
			$tapiceros = array('Florin','Javi','Paka','Mihai','Arturo','Tono','Augusto','Dani');
			foreach($tapiceros as $nom){
				//$objxlsx->getActiveSheet()->setCellValue('B'.$col,$nom);
				$objxlsx->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $nom);
				//$objxlsx->getActiveSheet()->setCellValueExplicitByColumnAndRow($col, $row, $nom, PHPExcel_Cell_DataType::TYPE_STRING); 
				/*if($col==0){
					$objxlsx->getActiveSheet()->getColumnDimensionByColumn($col)->setWidth(4);
				}elseif ($col==1){
					$objxlsx->getActiveSheet()->getColumnDimensionByColumn($col)->setWidth(4);
				}*/	
				$col++;
			}
			$objxlsx->getActiveSheet()->getStyleByColumnAndRow($col,$row)->getFont()->setBold(true);
			$objxlsx->getActiveSheet()->getStyleByColumnAndRow($col,$row)->getFont()->getColor()->setARGB('003030CF');
			$objxlsx->getActiveSheet()->getStyleByColumnAndRow($col,$row)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);  
			$objxlsx->getActiveSheet()->getStyleByColumnAndRow($col,$row)->getFill()->getStartColor()->setARGB('00FBF4A4');
		}
...
//************************************************************************************************************************
    ...
    { return array(
         ...
         'vo'=>array(),'mp'=>array(),'sc'=>array(),'cust'=>array(),'oc'=>array(),'idxr'=>array()
    );
    }  
//************************************************************************************************************************
...
//************************************************************************************************************************
   {if (count(self::$existf)){return;};//nacteno
	...
	//
	//*******************************************************************************
	//********Código añadido para descargar los artículos de idx*********************
	//*******************************************************************************
	if(in_array(_DB_PREFIX_."idxrcustomproduct_notes",$tindb)){
    $result = Db::getInstance()->ExecuteS("SHOW COLUMNS FROM `"._DB_NAME_."`."._DB_PREFIX_."idxrcustomproduct_notes");
    foreach($result as $res){self::$existf[] = 'idxr.'.$res['Field']; }    
    }  
	//******************************************************************************
	//******************************************************************************
	//****************************************************************************** 	
...
//************************************************************************************************************************
	...
	array('mp.personalization AS personalization',				$this->l('Personalizacion mega'),		115,1,1,1,0),
	//******************************************************************************
	//******************************************************************************
	//******************************************************************************
	//
	...
	//********Código añadido para obtener el transportista a travçes del estado************
	//*************************************************************************************
	array('sc.id_order_state AS carrier_status',						$this->l('Carrier by state'),		111,1,1,1,0),
	array('sc.date_add AS date_status',									$this->l('Date Status'),		112,1,1,1,0),
	//******************************************************************************
	//******************************************************************************
	//******************************************************************************
	//
	...
	//********Código añadido para obtener el transportista a travçes del estado************
	//*************************************************************************************
	array('cust.value AS customized_data',						$this->l('Personalizacion prod.'),		113,1,1,1,0),
	//******************************************************************************
	//******************************************************************************
	//******************************************************************************
	array('oc.comment AS order_comment',						$this->l('Order commnet'),				114,1,1,1,0),
	//******************************************************************************
	//********Código añadido para descargar los artículos de idx************
	//******************************************************************************
	array('idxr.public_note AS public_note',				   	$this->l('Atributos idxr'),	  116,1,1,1,0)
	//******************************************************************************
	//******************************************************************************
	//******************************************************************************
